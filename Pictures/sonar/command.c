...
extern char sonar_ip[100];
extern char eth_ip[100];
...
void set_sample_rate(void)
{
	...
	{
	sample_rate=-1;
	printf("Enter sample range range(200ns): ");
...
/*
brif@: send diffenet command and read response  from sonar unit.
	...
	if(strncmp(cmd,"W",1)==0)
		{
			...
			recvfrom(socketSD, rd_buff, 10000, 0, (struct sockaddr *)&receiveSockaddr, &receiveSockaddrLen);
			/*int i=0;
			while(rd_buff[i]!='\r')
				printf("%hhu\n",rd_buff[i++]);*/
		}
