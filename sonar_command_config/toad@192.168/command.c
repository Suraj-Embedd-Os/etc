...
void create_pcd_format(char *buff,int len)
{
	...
	float z_cordinat=0.0f;
	float rgb=0.0f;
	printf("saving  data in .pcd format\n");
	FILE *pcd_file=fopen("./sweep_data.pcd","a+");
	while(_step_size<_no_of_step)
	{
		x_cordinat=_range*cos(_step_size);
		y_cordinat=_range*sine(_step_size);
		fprintf("%f %f %f %f",x_cordinat,y_cordinat,z_cordinat,rgb)
		_step_size++;
	}
...
void set_cable_counter_ticks()
{
	...
	profile_survey(cmd,5,rd_data,10);
	//printf(" RECIVE :: %s \n",rd_data);
	if(!strncmp((char *)&rd_data,(char *)&cmd,1))
	...
	do{
		...
		system("clear");
			if(choice >=0 && choice <=10){
				profile_survey((unsigned char*)&center_angle[choice],rd_data,10);
				//printf(" RECIVE :: %s \n",rd_data);
...
void set_number_of_samples()
{
	...
	char temp=(((sonar_config->sample)>>8)&0xFF);
	 int i;
	for(i=0;i<=7;i++)
    {
...
void set_oversamples()
{
	...
	char cmd[5]={'\0'};
	cmd[0]='X';
	cmd[1]=(char)sonar_config->oversample;
	/*int os_val=-1;
	...
	do{
		...
		system("clear");
			if(choice >=0 && choice <=9){
				profile_survey((unsigned char*)&repeat_rate[choice],rd_data,10);
				//printf(" RECIVE :: %s \n",rd_data);
...
void set_sample_rate()
{
	...
	cmd[0]='V';
	 ...
	 char temp=(((sonar_config->samplerate)>>8)&0xFF);
	 int i;
	for(i=0;i<=7;i++)
    {
        cmd[1]|=(temp>>i&0x1)<<(7-i);
	...
	temp=((sonar_config->samplerate)&0xFF);
	for(i=0;i<=7;i++)
    {
        cmd[2]|=(temp>>i&0x1)<<(7-i);
    }
	...
	if(!strncmp((char *)&cmd,(char *)&rd_data,1)){
		printf("set sample range  sucessfull\n");
			}
			else
			{
...
void set_scan_arc_size()
{
	...
	do{
		...
		system("clear");
			if(choice >=0 && choice <=9){
				profile_survey((unsigned char*)&arc_size[choice],rd_data,10);
				//printf(" RECIVE :: %s \n",rd_data);
	...
	do{
		...
		system("clear");
			if(choice >=0 && choice <=3){
				profile_survey((unsigned char*)&step_size[choice],rd_data,10);
				//printf(" RECIVE :: %s \n",rd_data);
...
void set_tx_power()
{
	...
	return;	*/
		char cmd[5]={'\0'};
		strncpy(cmd,&sonar_config->txpower,1);
		profile_survey(cmd,1,rd_data,10);
		//printf(" RECIVE :: %s \n",rd_data);
	...
	do{
		...
		system("clear");
			if(choice >=0 && choice <=5){
				profile_survey((unsigned char*)&tx_pulse_width[choice],rd_data,10);
				//printf(" RECIVE :: %s \n",rd_data);
...
*/
void profile_survey(unsigned char * cmd,int cmd_len,char *rd_buff,int _size)
{
	printf("cmd get %s\n",cmd);
    struct sockaddr_in broadcastAddr;
    memset(&broadcastAddr, 0, sizeof(broadcastAddr));
   ...
   int ret = sendto(gsocketSD, (char *)request, strlen(request)+1, 0, (struct sockaddr*)&broadcastAddr, sizeof(broadcastAddr));
    if (ret < 0) {
        perror("sendto failed()");
        close(gsocketSD);
        return;
	...
	// receive ip ,device and mac informations
    ssize_t result = recvfrom(gsocketSD, rd_buff,  _size, 0, (struct sockaddr *)&receiveSockaddr, &receiveSockaddrLen);
