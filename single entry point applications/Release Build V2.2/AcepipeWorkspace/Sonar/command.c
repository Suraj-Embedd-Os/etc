...
extern int hight;
extern int width;
extern long unsigned int points;
...
void create_pcd_format(char *buff)
{
	...
	while(buff[i]!='\r')
	{
		...
		if(buff[i]!='+' || buff[i]!='-'){
		printf(" step_size %f x %f,y %f,z %f ,sweep_data %hhu  time %f\n",_angle,x_cordinat,y_cordinat,z_cordinat,buff[i],running_time);
		fprintf(pcd_file,"%f %f %f %hhu %f\n ",x_cordinat,y_cordinat,z_cordinat,buff[i],running_time);
...
void perform_sweep(void)
{
	...
	//FILE *fp=fopen("./sweep_data.txt","w");
