#ifndef SCANSETTINGSCALCULATOR_H_INCLUDED
#define SCANSETTINGSCALCULATOR_H_INCLUDED
...
class ScanSettingsCalculator
{
        ...
        /// <param name="vos">Velocity of sound in m/s</param>
          bool SetSamplingParameters(ScanSettings scanSettings, double range, uint16_t requestedSamples, double vos)
        {
            ...
            for (; sampleRate <= maxSampleRate; sampleRate += 20)
            {
                ...
                if (tuneSamples)
                {
                    ...
                    for (; sampleRate <= maxSampleRate; sampleRate += 20)
                    {
                        calculatedSamples = (ushort)(range / (sampleRate * pow(10.0, -9.0) * pow(2.0, oversamples) * vos / 2.0));
            ...
            if (actualRange > 0.0 && calculatedSamples < maxSamples)
            {
                ...
                scanSettings.Oversamples = (uint8_t)oversamples;
                scanSettings.RepeatRate = CalculateRepeatRate(calculatedSamples, (uint8_t)oversamples, sampleRate);
        ...
        /// <returns>true if ArcSize is valid; false if invalid</returns>
          bool VerifyArcSize(ArcSize &arcSize, CentreAngle centreAngle)
        {
            switch (centreAngle)
            {
        ...
        /// <returns>true if CentreAngle is valid</returns>
          bool VerifyCentreAngle(ArcSize arcSize,  CentreAngle &centreAngle)
        {
            switch (arcSize)
            {
          ...
          int GetPulseWidthMicroseconds(PulseWidth pulseWidth)
        {
            switch(pulseWidth)
            {
          ...
          uint16_t GetStepsPerPing(StepSize stepSize)
        {
            switch (stepSize)
            {
          ...
          uint16_t GetCentreAngleGradians(CentreAngle centreAngle)
        {
            switch (centreAngle)
            {
                case CentreAngle.centre30Degree:
          ...
          uint16_t GetHalfArcSizeGradians(ArcSize arcSize)
        {
            switch (arcSize)
            {
          ...
          double GetAngleFromArcSize(ArcSize arcSize, bool radians)
        {
        ...
        /// <returns>Angle in radians</returns>
          double GetAngleFromCentreAngle(CentreAngle centreAngle, bool radians)
        {
            double radiansPerDeg = 3.141 / 180.0;
            double angle = 0.0;
          ...
          uint16_t GetNumberOfPings(ArcSize arcSize, CentreAngle centreAngle, StepSize stepSize)
        {
            uint16_t stepsPerPing = GetStepsPerPing(stepSize);
        ...
        //region Static Methods
          RepeatRate CalculateRepeatRate(ushort samples, uint8_t oversamples, float sampleRate)
        {
        ...
        /// <returns></returns>
          RepeatRate GetRepeatRateEnum(double repeatRatems)
        {
        ...
        /// <returns></returns>
          double GetRepeatRateSeconds(RepeatRate repeatRate)
        {
            switch (repeatRate)
            {
        ...
        /// <returns>Step size in degrees</returns>
          double GetStepSizeDegrees(StepSize stepSize)
        {
            switch (stepSize)
            {
                case StepSize.step09Degree:
        ...
        /// <returns></returns>
          StepSize GetStepSizeEnum(double stepAngleDegrees)
        {
...
#endif // SCANSETTINGSCALCULATOR_H_INCLUDED
