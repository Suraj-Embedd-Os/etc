...
void get_sensor_data(void)
{
	...
	int ret=-1;
	double  pitch , roll;
	//char *file_name="/sensor_data.txt";
	char save_file[100];
	
	strcpy(save_file,path_to_save);
	strcat(save_file,timeBuff);
	
	printf("save_file  %s ",save_file);
	...
	if(!strncmp((char *)&rd_data,(char *)&cmd,1))
	{
		...
		//FILE *fp=fopen("/home/toad/AcepipeWorkspace/Sonar/sensor_data.txt","w"); //TODO 
		if(fp==NULL)
		{
...
void perform_sweep(void)
{
	...
	printf("save_file file %s ",save_file);
	...
	do{
		...
		system("clear");
			if(choice >=0 && choice <=10){
				profile_survey((unsigned char*)&center_angle[choice],1,rd_data,10);
				//printf(" RECIVE :: %s \n",rd_data);
...
void set_number_of_samples(void)
{
	char rd_data[10]={'\0'};
	 char cmd[10]={'\0'};
	 unsigned short no_sample=0;
	char temp=0;
	int key;
	do{
	no_sample=0;
	printf("\nEnter no of sample range(200 400 600) :\n");
	...
	do{
		...
		system("clear");
			if(choice >=0 && choice <=9){
				profile_survey((unsigned char*)&repeat_rate[choice],1,rd_data,10);
				//printf(" RECIVE :: %s \n",rd_data);
...
void set_scan_arc_size(void)
{
	...
	do{
		...
		system("clear");
			if(choice >=0 && choice <=9){
				profile_survey((unsigned char*)&arc_size[choice],1,rd_data,10);
				//printf(" RECIVE :: %s \n",rd_data);
	...
	do{
		...
		system("clear");
			if(choice >=0 && choice <=3){
				profile_survey((unsigned char*)&step_size[choice],1,rd_data,10);
				//printf(" RECIVE :: %s \n",rd_data);
	...
	do{
		...
		system("clear");
			if(choice >=0 && choice <=5){
				profile_survey((unsigned char*)&tx_pulse_width[choice],1,rd_data,10);
				//printf(" RECIVE :: %s \n",rd_data);
...
void step_motor_anti_clockwise(void)
{
	...
	char cmd[]={'C'};
	profile_survey(cmd,1,rd_data,10);
	if(!strncmp(cmd,rd_data,1))
		printf("Moves the underwater unit's motor 1 step Anti-clockwise\n");
	else
		printf("Not able to move motor in Anti-clockwise\n");
}
/*
...
void step_motor_clockwise(void)
{
	...
	profile_survey(cmd,1,rd_data,10);
	if(!strncmp(cmd,rd_data,1))
		printf("Moves the underwater unit's motor 1 step clockwise\n");
	else
		printf("Not able to move motor in clockwise\n");
}
...
void profile_survey(unsigned char * cmd,int _cmd_len,char *rd_buff,int _size)
{
    ...
    ret = sendto(socketSD, (char *)request,_cmd_len, 0, (struct sockaddr*)&broadcastAddr, sizeof(broadcastAddr));
    if (ret < 0) {
        perror("sendto failed()");
        close(socketSD);
        return;
   ...
   // char buf[bufSize];
	...
	// receive ip ,device and mac informations
    ssize_t result = recvfrom(socketSD, (unsigned char*)&rd_buff[0],  _size, 0, (struct sockaddr *)&receiveSockaddr, &receiveSockaddrLen);
