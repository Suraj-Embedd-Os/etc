#!/bin/bash

cd /mnt/lfs/sources

wget https://download.savannah.gnu.org/releases/acl/acl-2.3.1.tar.xz
wget https://download.savannah.gnu.org/releases/attr/attr-2.5.1.tar.gz
wget https://ftp.gnu.org/gnu/autoconf/autoconf-2.71.tar.xz
wget https://ftp.gnu.org/gnu/automake/automake-1.16.5.tar.xz
wget https://ftp.gnu.org/gnu/bash/bash-5.2.15.tar.gz
wget https://github.com/gavinhoward/bc/releases/download/6.6.0/bc-6.6.0.tar.xz
wget https://sourceware.org/pub/binutils/releases/binutils-2.40.tar.xz
wget https://ftp.gnu.org/gnu/bison/bison-3.8.2.tar.xz
wget https://github.com/libcheck/check/releases/download/0.15.2/check-0.15.2.tar.gz
wget https://ftp.gnu.org/gnu/coreutils/coreutils-9.3.tar.xz
wget https://ftp.gnu.org/gnu/dejagnu/dejagnu-1.6.3.tar.gz
wget https://ftp.gnu.org/gnu/diffutils/diffutils-3.10.tar.xz
wget https://downloads.sourceforge.net/project/e2fsprogs/e2fsprogs/v1.47.0/e2fsprogs-1.47.0.tar.gz
wget https://sourceware.org/ftp/elfutils/0.189/elfutils-0.189.tar.bz2
wget https://github.com/eudev-project/eudev/releases/download/v3.2.12/eudev-3.2.12.tar.gz
wget https://prdownloads.sourceforge.net/expat/expat-2.5.0.tar.xz
wget https://prdownloads.sourceforge.net/expect/expect5.45.4.tar.gz
wget https://astron.com/pub/file/file-5.44.tar.gz
wget https://ftp.gnu.org/gnu/findutils/findutils-4.9.0.tar.xz
wget https://github.com/westes/flex/releases/download/v2.6.4/flex-2.6.4.tar.gz
wget https://pypi.org/packages/source/f/flit-core/flit_core-3.9.0.tar.gz
wget https://ftp.gnu.org/gnu/gawk/gawk-5.2.2.tar.xz
wget https://ftp.gnu.org/gnu/gcc/gcc-13.1.0/gcc-13.1.0.tar.xz
wget https://ftp.gnu.org/gnu/gdbm/gdbm-1.23.tar.gz
wget https://ftp.gnu.org/gnu/gettext/gettext-0.21.1.tar.xz
wget https://ftp.gnu.org/gnu/glibc/glibc-2.37.tar.xz
wget https://ftp.gnu.org/gnu/gmp/gmp-6.2.1.tar.xz
wget https://ftp.gnu.org/gnu/gperf/gperf-3.1.tar.gz
wget https://ftp.gnu.org/gnu/grep/grep-3.11.tar.xz
wget https://ftp.gnu.org/gnu/groff/groff-1.22.4.tar.gz
wget https://ftp.gnu.org/gnu/grub/grub-2.06.tar.xz
wget https://ftp.gnu.org/gnu/gzip/gzip-1.12.tar.xz
wget https://github.com/Mic92/iana-etc/releases/download/20230524/iana-etc-20230524.tar.gz
wget https://ftp.gnu.org/gnu/inetutils/inetutils-2.4.tar.xz
wget https://launchpad.net/intltool/trunk/0.51.0/+download/intltool-0.51.0.tar.gz
wget https://www.kernel.org/pub/linux/utils/net/iproute2/iproute2-6.3.0.tar.xz
wget https://www.kernel.org/pub/linux/utils/kbd/kbd-2.6.0.tar.xz
wget https://www.kernel.org/pub/linux/utils/kernel/kmod/kmod-30.tar.xz
wget https://www.greenwoodsoftware.com/less/less-633.tar.gz
wget https://www.linuxfromscratch.org/lfs/downloads/development/lfs-bootscripts-20230101.tar.xz
wget https://www.kernel.org/pub/linux/libs/security/linux-privs/libcap2/libcap-2.69.tar.xz
wget https://github.com/libffi/libffi/releases/download/v3.4.4/libffi-3.4.4.tar.gz
wget https://download.savannah.gnu.org/releases/libpipeline/libpipeline-1.5.7.tar.gz
wget https://ftp.gnu.org/gnu/libtool/libtool-2.4.7.tar.xz
wget https://www.kernel.org/pub/linux/kernel/v6.x/linux-6.3.8.tar.xz
wget https://ftp.gnu.org/gnu/m4/m4-1.4.19.tar.xz
wget https://ftp.gnu.org/gnu/make/make-4.4.1.tar.gz
wget https://download.savannah.gnu.org/releases/man-db/man-db-2.11.2.tar.xz
wget https://www.kernel.org/pub/linux/docs/man-pages/man-pages-6.04.tar.xz
wget https://github.com/mesonbuild/meson/releases/download/1.1.1/meson-1.1.1.tar.gz
wget https://ftp.gnu.org/gnu/mpc/mpc-1.3.1.tar.gz
wget https://ftp.gnu.org/gnu/mpfr/mpfr-4.2.0.tar.xz
wget https://invisible-mirror.net/archives/ncurses/ncurses-6.4.tar.gz
wget https://github.com/ninja-build/ninja/archive/v1.11.1/ninja-1.11.1.tar.gz
wget https://www.openssl.org/source/openssl-3.1.1.tar.gz
wget https://ftp.gnu.org/gnu/patch/patch-2.7.6.tar.xz
wget https://www.cpan.org/src/5.0/perl-5.36.1.tar.xz
wget https://distfiles.dereferenced.org/pkgconf/pkgconf-1.9.5.tar.xz
wget https://sourceforge.net/projects/procps-ng/files/Production/procps-ng-4.0.3.tar.xz
wget https://sourceforge.net/projects/psmisc/files/psmisc/psmisc-23.6.tar.xz
wget https://www.python.org/ftp/python/3.11.4/Python-3.11.4.tar.xz
wget https://www.python.org/ftp/python/doc/3.11.4/python-3.11.4-docs-html.tar.bz2
wget https://ftp.gnu.org/gnu/readline/readline-8.2.tar.gz
wget https://ftp.gnu.org/gnu/sed/sed-4.9.tar.xz
wget https://github.com/shadow-maint/shadow/releases/download/4.13/shadow-4.13.tar.xz
wget https://www.infodrom.org/projects/sysklogd/download/sysklogd-1.5.1.tar.gz
wget https://github.com/slicer69/sysvinit/releases/download/3.07/sysvinit-3.07.tar.xz
wget https://ftp.gnu.org/gnu/tar/tar-1.34.tar.xz
wget https://downloads.sourceforge.net/tcl/tcl8.6.13-src.tar.gz
wget https://downloads.sourceforge.net/tcl/tcl8.6.13-html.tar.gz
wget https://ftp.gnu.org/gnu/texinfo/texinfo-7.0.3.tar.xz
wget https://www.iana.org/time-zones/repository/releases/tzdata2023c.tar.gz
wget https://anduin.linuxfromscratch.org/LFS/udev-lfs-20171102.tar.xz
wget https://www.kernel.org/pub/linux/utils/util-linux/v2.39/util-linux-2.39.tar.xz
wget https://anduin.linuxfromscratch.org/LFS/vim-9.0.1503.tar.gz
wget https://pypi.org/packages/source/w/wheel/wheel-0.40.0.tar.gz
wget https://cpan.metacpan.org/authors/id/T/TO/TODDR/XML-Parser-2.46.tar.gz
wget https://tukaani.org/xz/xz-5.4.3.tar.xz
wget https://zlib.net/zlib-1.2.13.tar.xz
wget https://github.com/facebook/zstd/releases/download/v1.5.5/zstd-1.5.5.tar.gz
